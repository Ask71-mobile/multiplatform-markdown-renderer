...
import com.mikepenz.markdown.utils.MARKDOWN_TAG_SOURCE
...
/**
 * Processes source placeholders in the text and converts them to inline content.
 *
 * @param text The text to process for source placeholders.
 */
fun AnnotatedString.Builder.processSourceTags(text: String) {
    val sourcePlaceholderPattern = Regex("""§SOURCE:([^:]*):([^§]*)§""")
    var lastIndex = 0
    
    sourcePlaceholderPattern.findAll(text).forEach { match ->
        // Append text before the source placeholder
        if (match.range.first > lastIndex) {
            append(text.substring(lastIndex, match.range.first))
        }
        
        // Extract name and content from the placeholder
        val name = match.groupValues[1]
        val content = match.groupValues[2]
        val sourceData = "$name|$content"
        
        // Append inline content for the source chip
        appendInlineContent(MARKDOWN_TAG_SOURCE, sourceData)
        
        lastIndex = match.range.last + 1
    }
    
    // Append any remaining text after the last source placeholder
